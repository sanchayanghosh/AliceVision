...
int aliceVision_main(int argc, char **argv)
{
  ...
  bool noColorProfileIsError = true;
  ...
  optionalParams.add_options()
    ...
    ("noColorProfileIsError", po::value<bool>(&noColorProfileIsError)->default_value(noColorProfileIsError),
...
#pragma omp critical
    ...
    if (!colorProfileDatabaseDirPath.empty() && (imgFormat.compare("raw") == 0))
    {
        ...
        #pragma omp critical
        {
            allColorProfilesFound = allColorProfilesFound && colorProfileFound;
        }
    }
